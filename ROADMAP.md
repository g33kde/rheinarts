...
- [ ] **Grow the arcade.** Replace the "coming soon" cabinets with real games as
  ...
  _(in progress — Godspeed has a cabinet, marked `BETA` (orange badge, distinct
  from HyperOut's cyan `PLAYABLE`), linking to `/godspeed/index.html`. Debris
  now has one too, marked `PLAYABLE` like HyperOut (not `BETA` - see
  `debris/CHANGELOG.md`), linking to `/debris/index.html`. No "coming soon"
  slots remain. "About Rhein Arts" blurb and DE/EN toggle still not done.)_
